 ...
 * @since           1.0.0
 * @package         Wp1
 * @subpackage      Wp1/widgets
 * @author          Ajinkya <ajinkya.khutwad@hbwsl.com>
 ...
 * @return void
 */
function wp1_book_widget_init() {

	register_widget( 'WP1_Book_Widget' );
}

/**
 * Undocumented class
 */
class WP1_Book_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct( 'wp1_book_widget', 'Book Widget', $widget_ops );
	}

	/**
	 * Allows parsing of html tags and its attributes.
	 */
	public function allow_html_tags() {
		return array(
			'div' => array(
				'class' => array(),
			),
	...
	public function widget( $args, $instance ) {
		...
		// Display text field.
		if ( true === isset( $text ) ) {
			echo wp_kses( $args['before_widget'], $this->allow_html_tags() ) . wp_kses( $args['before_title'], $this->allow_html_tags() ) . wp_kses( $text, $this->allow_html_tags() ) . wp_kses( $args['after_title'], $this->allow_html_tags() );
		}

		// Display select field.
		$id_count = 0;
		if ( true === isset( $select ) ) {
			$args  = array(
				'post_type'   => 'book',
				'post_status' => 'publish',
				'tax_query'   => array(
